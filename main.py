...
class AONet:
    ...
    def train(self, output_dir='EX-0',splitfilename=None,splitid=None):
        viz = Visdom(env=splitfilename.split('/')[-1]+"_"+str(splitid))
        viz.line([0.], [0.], win='train_loss', opts=dict(title='train loss'))
        viz.line([[0.0, 0.0]], [0.], win='F-score',
                 opts=dict(title='avg/max F-score',
                           legend=['avg_score', 'max_score']))
        '''viz.line([0.], [0.], win='max_norm', opts=dict(title='max norm'))'''
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            viz.line([np.mean(avg_loss[:, 0])], [epoch], win='train_loss', update='append')
            viz.line([[val_fscore, max_val_fscore]], [epoch], win='F-score', update='append')
    ...
    def eval(self, keys, result_txt=None, split_id=None, results_filename=None):
        ...
        f_score, video_scores = self.eval_summary(summary, keys, metric=self.dataset_name,
                    split_id=split_id,results_filename=results_filename, att_vecs=att_vecs)

        if result_txt!=None:
            result_txt.write(str(summary))
            result_txt.write("\n\n")
        return f_score, video_scores


    def eval_summary(self, machine_summary_activations, test_keys, split_id=None, results_filename=None, metric='tvsum', att_vecs=None):

        eval_metric = 'avg' if metric == 'tvsum' else 'max'

        if results_filename is not None:
            h5_res = h5py.File(results_filename, 'a')
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            fm, _, _ = evaluate_summary(machine_summary, user_summary, eval_metric)#fm:final_f_score
            fms.append(fm)
            ...
            if results_filename:
                gt = d['gtscore'][...]
                h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/score', data=probs)
                h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/machine_summary', data=machine_summary)
                h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/gtscore', data=gt)
                h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/fm', data=fm)
                h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/picks', data=positions)

                video_name = key.split('/')[1]
                if 'video_name' in d:
                    video_name = d['video_name'][...]
                h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/video_name', data=video_name)

                if att_vecs is not None:
                    h5_res.create_dataset(key.split("/")[1] + '/'+ str(split_id) + '/att', data=att_vecs[key])
...
def eval_split(hps, splits_filename, data_dir='test',result_txt=None):
    ...
    result_h5_path = "result/" + splits_filename.split("/")[1][0:-11] + "result.h5"
    ...
    for split_id in range(len(ao.splits)):
        ...
        val_fscore, video_scores = ao.eval(ao.test_keys,result_txt=result_txt, split_id=split_id, results_filename=result_h5_path)
...
def train(hps):
    ...
    os.system('copy splits\*.json ' + hps.output_dir + '\splits /y')
    os.system('copy *.py ' + hps.output_dir + '\code')
    ...
    for split_filename in hps.splits:
        ...
        for split_id in range(n_folds):
            ...
            fscore, fscore_epoch = ao.train(output_dir=hps.output_dir,splitfilename=split_filename,splitid=split_id)
            f_avg += fscore
            ...
            os.system('rd/s/q ' + hps.output_dir + '\\models_temp\\' + log_dir)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-v', '--verbose', action='store_false', help="Prints out more messages")
    ...
    if hps.train:
        ...
        for i, split_filename in enumerate(hps.splits):
            f_score = eval_split(hps, split_filename, data_dir=hps.output_dir,result_txt=result_txt)
